...
/**
 * @brief      This function process the received packet in 1mbps s b mode only for hanshow for the
 ...
 * @param[in]  len - the expected rx length of s b mode, containing payload and 3byte crc
...
/**
 * @brief      This function process the tx packet in 1mbps s b mode only for hanshow for the
 *             compatiblity with third-party chips. The process includes data-whitening
               transformation and crc padding.
 * @param[in]  tx_buf - the rf tx buffer containing the tx packet(dma length+payload+3 byte crc)
 * @param[in]  len - the expected tx length of s b mode, containing payload and 3byte crc
 * @return     none
 */
