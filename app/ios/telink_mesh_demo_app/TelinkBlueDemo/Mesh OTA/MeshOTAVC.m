...
@property (nonatomic, assign) BOOL isGattOTA;
...
- (void)startMeshOTA{
    ...
    NSData *binData = self.binDataArray[self.binIndex];
    UInt16 binType = [self getPidWithOTAData:binData];
    MeshOTAItemModel *model = self.array[self.itemIndex];
    if (model.deviceType != binType) {
        [UIAlertView alertWithMessage:@"请选择与类型匹配bin文件"];
        return;
    }
    ...
    for (DeviceModel *device in self.collectionSource) {
        if (device.stata != LightStataTypeOutline) {
            [onlineDevices addObject:device];
        }
    }
    __weak typeof(self) weakSelf = self;
