...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error{
    ...
    for (CBService *tempSer in peripheral.services)
    {
        if ([tempSer.UUID isEqual:[CBUUID UUIDWithString:BTDevInfo_ServiceUUID]]){
            ...
            [self printContentWithString:[NSString stringWithFormat:@"did discover services for address: 0x%04x uuid=%@", self.selConnectedItem.u_DevAdress,peripheral.identifier.UUIDString]];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    ...
    [self printContentWithString:[NSString stringWithFormat:@"did discover characteristics for address: 0x%04x uuid=%@", self.selConnectedItem.u_DevAdress,peripheral.identifier.UUIDString]];
...
-(void)startScanWithName:(NSString *)nStr Pwd:(NSString *)pwd{
    ...
    if (_centerState == CBCentralManagerStatePoweredOn) {
        ...
        NSLog(@"[CoreBluetooth] ready scan devices.");
...
/*
 packet of end OTA 4 bytes structure：2bytes:index + 2bytes:CRC
 */
